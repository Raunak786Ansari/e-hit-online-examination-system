...
public class RightPanel extends JPanel {
	...
	void createRightPanel(String[][] details, byte[] b) throws Exception {
		...
		// scaling the image
		/*
		 * BufferedImage toolkitImage = bi.getScaledInstance(130,
		 * 150,BufferedImage.SCALE_SMOOTH);
			...
			if (e.getSource() == (Object) cPassword) {
				...
				if (editStatus == false) {
					...
					for (int index = 0; index < n; index++) {
						if (!label1[index].getText().startsWith("Id No")
								&& !label1[index].getText().startsWith("Group(s)")) {
					...
					String[] queries = {
							"UPDATE instructor SET first_name = '" + first_name + "' WHERE iid=" + Dashboard.iid,
							...
							"UPDATE instructor SET institute_name = '" + institute_name + "' WHERE iid="
									+ Dashboard.iid,
