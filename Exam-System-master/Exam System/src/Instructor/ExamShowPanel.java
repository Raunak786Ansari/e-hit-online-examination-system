...
public class ExamShowPanel extends JPanel {
	...
	void getExams() {
		...
		/*
		 * exams = new
		 * String[][]{{"E1","Avul Pakir Jainulabdeen Abdul Kalam Exam","25-04-2016"},
		...
		// qry2 = "SELECT eid,ename,edate_time FROM exam WHERE exam.iid=
			...
			if (choice == 0) {
				...
				for (int i = 0; i < n; i++) {
					...
					if (e.getSource() == (Object) deleteExamButton[i]) {
						 ...
						 * "where qid in (SELECT Exam.qid from Exam,ExamQuestionTable Where Exam.Eid
						 * =,ExamQuestionTable.Eid
						 * and Exam.eid = examid
						 */
